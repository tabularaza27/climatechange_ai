...
<table class='remote-workshop-table'>
  ...
  <tbody>
  {% for r in site.data.icml2021_schedule %}
      ...
      <ul>
        {% for p in site.data.icml2021_papers %}
          {% if p.q1_track == track and p.poster_sessions contains session %}
            <li><a href='/papers/icml2021/{{p.id}}' target='_blank'>({{p.id}}) {{p.paper_title}}</a></li>
          {% endif %}
        {% endfor %}
      </ul>
    {% endfor %}
  ...
  <td>
    ...
    {% assign p = site.data.icml2021_papers[idx] %}
    <a href="/papers/icml2021/{{ p.id }}" target="_blank">
